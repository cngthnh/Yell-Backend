...
def sendVerificationEmail(recipient, token, recipient_name, code):
    ...
    msg = Message(subject = 'Yell Account Verification', sender = ('Yell App', sender_mail), recipients = [recipient])
...
def sendDashboardInvitation(token, recipient_email, recipient_name, dashboard_name):
    sender_mail = os.environ.get('MAIL_USERNAME')
    msg = Message(subject = 'Yell Dashboard Invitation', sender = ('Yell App', sender_mail), recipients = [recipient_email])
...
def sendPasswordChangeRequest(recipient, recipient_name, code):
    sender_mail = os.environ.get('MAIL_USERNAME')
    msg = Message(subject = 'Yell Password Change Confirmation', sender = ('Yell App', sender_mail), recipients = [recipient])
