    ...
    def tokenCheck(*args, **kwargs):
        ...
        try:
            ...
            if (tokenDict[API_TOKEN_TYPE]!=ACCESS_TOKEN_TYPE):
                return getMessage(message=ACCESS_TOKEN_REQUIRED_MESSAGE), 403

            session = db.session.query(Session).filter_by(id=tokenDict[SESSION_ID_KEY]).first()

            if session is None:
                return getMessage(message=INVALID_SESSION_MESSAGE), 403
...
def getToken():
    try:
        ...
        print(data)
        sys.stdout.flush()
        return getMessage(message=INVALID_CREDENTIALS_MESSAGE), 401
    ...
    try:
        ...
        return getMessage(message=INACTIVATED_ACCOUNT_MESSAGE), 401
    except Exception as e:
        print(e)
        sys.stdout.flush()
        return getMessage(message=INVALID_CREDENTIALS_MESSAGE), 401
...
def refreshToken():
    ...
    try:
        ...
        if (tokenDict[API_TOKEN_TYPE]!=REFRESH_TOKEN_TYPE):
            return getMessage(message=REFRESH_TOKEN_REQUIRED_MESSAGE), 403
